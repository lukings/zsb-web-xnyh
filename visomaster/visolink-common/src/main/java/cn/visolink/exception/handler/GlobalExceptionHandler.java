...
public class GlobalExceptionHandler {
    ...
    private ResultBody buildResponseEntity(ApiError apiError) {
        if (apiError.getMessages()!=null && !"".equals(apiError.getMessages()) && (apiError.getMessages().contains("Exception") || apiError.getMessages().length()>100)){
            apiError.setMessages("发生异常！");
        }
