...
//        }
     ...
     * @return
     */
    public static JSONObject httpHeaderPost(String url, JSONObject jsonParam, boolean noNeedResponse) {
...
//        }
       HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest();
        String token = request.getHeader("X-AURORA-TOKEN");
        //post请求返回结果
        DefaultHttpClient httpClient = new DefaultHttpClient();
        JSONObject jsonResult = null;
        HttpPost method = new HttpPost(url);
        method.setHeader("X-AURORA-TOKEN",token);
        try {
            if (!StringUtils.isEmpty(jsonParam)) {
                //解决中文乱码问题
                StringEntity entity = new StringEntity(jsonParam.toString(), "utf-8");
...
//            HttpServletRequest requestOld = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest();
//            String companycode = requestOld.getHeader("companycode");

            //发送get请求
            HttpGet request = new HttpGet(url);
//            if (!StringUtils.isEmpty(companycode)){
//                request.setHeader("companycode",companycode);
//            }
