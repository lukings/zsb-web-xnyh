...
public class CustTaskMapController {
     ...
     **/
    @PostMapping("/rwMapT")
    public ResultBody rwMapT(@RequestBody Map paramMap) {
...
//        }
        ...
        if (!CollectionUtils.isEmpty(clueStatusList)) {
            ...
            if (isSales) {
                ...
                //查询线索
                if (clueStatusList.contains("3") || clueStatusList.contains("5") || clueStatusList.contains("8") || clueStatusList.contains("9")) {
                    List<ZsMapResBO> clues = custMapDao.getCluesTkdt(paramMap);
                    List<ZsMapResBO> cluesHis = custMapDao.getCluesTkdtHis(paramMap);
                    clues.addAll(cluesHis);
                    //去掉在保客户查出来的目标企业
                    ...
                    //目标企业 b_task_customer 中 customerType！=3  就是未拓客户的数量
                    List<ZsMapResBO> s8_3 = clues.stream()
                            .filter(a -> "8".equals(a.getType()))           // 保留 type 等于 "8" 的元素--目标企业
                            .filter(a -> !"3".equals(a.getCustomerType()))  // 保留 customerType 不等于 "3" 的 任务新增客户
                            .collect(Collectors.toList());
            ...
            if (isMap) {
                ...
                if (!CollectionUtils.isEmpty(clueMap) && clueMap.size() > 0) {
                    ...
                    str.append("目标客户有" + clueMap.size() + "组;");
                }
            }
        }
        result.put("zsMapResBOList", zsMapResBOList);
        result.put("message", str.toString());
        result.put("xfkh", zsMapResBOList.size());
        result.put("ytkh", zsMapResBOList.size()-wtkh);
    ...
    private List<String> initClueStatus(String clueStatus, String taskName, String flagType) {
        ...
        if (StringUtils.isNotBlank(flagType)) {
            if ("1".equals(flagType)) {//在保客户
                //clueStatusList = Stream.of("1", "2", "4", "6", "7").collect(Collectors.toList());
                //任务查询默认传1，要查在保和线索
