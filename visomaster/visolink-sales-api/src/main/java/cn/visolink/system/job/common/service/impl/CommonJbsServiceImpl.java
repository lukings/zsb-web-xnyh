...
public class CommonJbsServiceImpl extends ServiceImpl<CommonJobsDao, CommonJobs> implements CommonJobsService {
    ...
    @Value("${outbound.addRoleOp}")
    private String addRoleOp;
    ...
    public PageInfo<CommonJobsVO> commonJobsSelectAll(CommonJobsForm commonjobsForm) {
        PageHelper.startPage((int)commonjobsForm.getCurrent(), (int)commonjobsForm.getSize());
        com.github.pagehelper.Page<CommonJobsVO> list = baseMapper.findListBySystemCommonJobs(commonjobsForm);
    ...
    public int systemCommonJob_Insert(CommonJobsForm commonJobsForm) {
        ...
        commonJobsForm.setId(id);
        Map<String, Object> map = new HashMap<>();
        map.put("title",commonJobsForm.getJobName());
        map.put("three_id",id);
        map.put("type",4);

        HttpClientUtil.postHttpOutbound(addRoleOp,redisService.getVal("outbound."+commonJobsForm.getEditor())+"",map);
