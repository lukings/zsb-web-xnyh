...
public class CustZsMapControllerT {
    ...
    @PostMapping("/zsMapT")
    public ResultBody zsMapT(@RequestBody Map<String, Object> paramMap) {
        // 1. 初始化基础参数
    ...
    private void performAllQueries(Map<String, Object> paramMap, Map<String, Object> result) {
        ...
        String dataRange = (String) paramMap.get("dataRange");
    ...
    private List<ZsMapResBO> queryMapImportCustomers(Map paramMap, List<String> clueStatusList) {
        List<ZsMapResBO> clues = custMapDao.getCluesFromCache(paramMap);
        List<ZsMapResBO> cluesHis = new ArrayList<>();
        if(paramMap.get("mapType").toString().equals("2")){
            cluesHis = custMapDao.getCluesTkdtHis(paramMap);
        }else{
            cluesHis = custMapDao.getCluesKhdtHis(paramMap);
        }
        if (StringUtils.isNotEmpty((String) paramMap.get("latLon"))) {
            clues = filterByLatLon(clues, (String) paramMap.get("latLon"));
