...
public class CustTkMapController {
...
//        // 清空旧数据（可选，也可以使用更新策略）
//        custMapDao.clearCustomerStatusCache();
//
//        // 执行状态计算并插入缓存表
//        custMapDao.updateCustomerStatusCacheXM();
//        custMapDao.updateCustomerStatusCacheQY();
//        custMapDao.updateCustomerStatusCacheJT();
//        return "调用成功！！";
//    }
     ...
     **/
    @PostMapping("/tkMap")
    public ResultBody tkMap(@RequestBody Map paramMap) {
...
//        }
        ...
        if (!CollectionUtils.isEmpty(clueStatusList)) {
            ...
            if (isSales) {
                if (clueStatusList.contains("1") || clueStatusList.contains("2") || clueStatusList.contains("7") || clueStatusList.contains("6")  || clueStatusList.contains("4")  || clueStatusList.contains("8")) {
                    // 使用缓存表查询替代原来的直接查询
                    ...
                    List<ZsMapResBO> s7 = opportunitys.stream()
                            .filter(a -> "7".equals(a.getType())).collect(Collectors.toList());
                    List<ZsMapResBO> s6 = opportunitys.stream()
                    ...
                    List<ZsMapResBO> s8 = opportunitys.stream()
                    ...
                    if (!CollectionUtils.isEmpty(s1) && s1.size() > 0 && clueStatusList.contains("1")) {
                        str.append(ClueAcquisitionStatusEnum.getNameByStatus("1") + "的客户有"
                                + s1.size() + "组;");
                        zsMapResBOList.addAll(s1);
                    }
                    if (!CollectionUtils.isEmpty(s2) && s2.size() > 0 && clueStatusList.contains("2")) {
                        str.append(ClueAcquisitionStatusEnum.getNameByStatus("2") + "的客户有"
                                + s2.size() + "组;");
                        zsMapResBOList.addAll(s2);
                    }
                    if (!CollectionUtils.isEmpty(s7) && s7.size() > 0 && clueStatusList.contains("7")) {
                        str.append(ClueAcquisitionStatusEnum.getNameByStatus("7") + "的客户有"
                                + s7.size() + "组;");
                        zsMapResBOList.addAll(s7);
                    }
                    if (!CollectionUtils.isEmpty(s6) && s6.size() > 0 && clueStatusList.contains("6")) {
                        str.append(ClueAcquisitionStatusEnum.getNameByStatus("6") + "的客户有"
                                + s6.size() + "组;");
                        zsMapResBOList.addAll(s6);
                    }
                    if (!CollectionUtils.isEmpty(s4) && s4.size() > 0 && clueStatusList.contains("4")) {
                        str.append(ClueAcquisitionStatusEnum.getNameByStatus("4") + "的客户有"
                                + s4.size() + "组;");
                        zsMapResBOList.addAll(s4);
                    }
                    if (!CollectionUtils.isEmpty(s8) && s8.size() > 0 && clueStatusList.contains("8")) {
                        str.append(ClueAcquisitionStatusEnum.getNameByStatus("8") + "（转报备）的客户有"
                                + s8.size() + "组;");
                        zsMapResBOList.addAll(s8);
                    }

                }
                //查询线索
                if (clueStatusList.contains("3") || clueStatusList.contains("5") || clueStatusList.contains("9")||  clueStatusList.contains("8")) {
                    List<ZsMapResBO> clues = custMapDao.getCluesFromCache(paramMap);


                    // 记录DAO查询信息
                    daoInfo = new HashMap<>();
                    daoInfo.put("dao名称", "custMapDao.getCluesTkdt");
                    ...
                    List<ZsMapResBO> s3 = clues.stream()
                            .filter(a -> "3".equals(a.getType())).collect(Collectors.toList());
                    List<ZsMapResBO> s5 = clues.stream()
                            .filter(a -> "5".equals(a.getType())).collect(Collectors.toList());
                    List<ZsMapResBO> s9 = clues.stream()
                            .filter(a -> "9".equals(a.getType())).collect(Collectors.toList());
                    List<ZsMapResBO> s8 = clues.stream()
                            .filter(a -> "8".equals(a.getType())).collect(Collectors.toList());

                    if (!CollectionUtils.isEmpty(s3) && s3.size() > 0 && clueStatusList.contains("3")) {
                        str.append(ClueAcquisitionStatusEnum.getNameByStatus("3") + "的客户有"
                                + s3.size() + "组;");
                        zsMapResBOList.addAll(s3);
                    }
                    if (!CollectionUtils.isEmpty(s5) && s5.size() > 0 && clueStatusList.contains("5")) {

                        str.append(ClueAcquisitionStatusEnum.getNameByStatus("5") + "的客户有"
                                + s5.size() + "组;");
                        zsMapResBOList.addAll(s5);
                    }
                    if (!CollectionUtils.isEmpty(s9) && s9.size() > 0 && clueStatusList.contains("s9")) {

                        str.append(ClueAcquisitionStatusEnum.getNameByStatus("9") + "的客户有"
                                + s9.size() + "组;");
                        zsMapResBOList.addAll(s9);
                    }
                    if (!CollectionUtils.isEmpty(s8) && s8.size() > 0 && clueStatusList.contains("8")) {

                        str.append(ClueAcquisitionStatusEnum.getNameByStatus("8") + "的客户有"
                                + s8.size() + "组;");
                        zsMapResBOList.addAll(s8);
                    }
                }
                // 在返回结果前，确保每个对象都有mapType和dataRange值
                for (ZsMapResBO bo : zsMapResBOList) {
                    if (StringUtils.isEmpty(bo.getMapType())) {
                        bo.setMapType(mapType);
                    }
                    if (StringUtils.isEmpty(bo.getDataRange())) {
                        bo.setDataRange(dataRange);
                    }
                }
            }
            //公客池
            ...
            if (isMap) {
                ...
                //只能下发目标企业
                if(StringUtils.isNotBlank(isTaskCreate) && "1".equals(isTaskCreate)) {
                    clues = clues.stream()
                            .filter(a -> "8".equals(a.getType())).collect(Collectors.toList());
                    cluesHis = cluesHis.stream()
                            .filter(a -> "8".equals(a.getType())).collect(Collectors.toList());
                ...
                List<ZsMapResBO> s1 = cluesHis.stream()
                        .filter(a -> "1".equals(a.getType())).collect(Collectors.toList());
                List<ZsMapResBO> s2 = cluesHis.stream()
                        .filter(a -> "2".equals(a.getType())).collect(Collectors.toList());
                List<ZsMapResBO> s4 = cluesHis.stream()
                        .filter(a -> "4".equals(a.getType())).collect(Collectors.toList());
                //已成交1-8
                ...
                List<ZsMapResBO> s3 = cluesHis.stream()
                        .filter(a -> "3".equals(a.getType())).collect(Collectors.toList());
                List<ZsMapResBO> s5 = cluesHis.stream()
                        .filter(a -> "5".equals(a.getType())).collect(Collectors.toList());
                List<ZsMapResBO> s8 = cluesHis.stream()
                        .filter(a -> "8".equals(a.getType())).collect(Collectors.toList());
                ...
                List<ZsMapResBO> clueMap = clues.stream()
                        ...
                        .collect(Collectors.toList());
                //去掉在保客户查出来的目标企业
                Set<String> zsMapResBOListS8 = zsMapResBOList.stream()
                        .map(ZsMapResBO::getProjectClueId)
                        .filter(Objects::nonNull)  // 可选：过滤空值
                        .collect(Collectors.toSet());
                clueMap = clueMap.stream()
                        .filter(a -> "8".equals(a.getType()))
                        .filter(a -> !zsMapResBOListS8.contains(a.getProjectClueId()))
                        .collect(Collectors.toList());

                //历史数据中包好【目标企业】
                if (!CollectionUtils.isEmpty(s8) && s8.size() > 0 && clueStatusList.contains("8") && !CollectionUtils.isEmpty(clueMap) && clueMap.size() > 0) {
                    str.append(ClueAcquisitionStatusEnum.getNameByStatus("8") + "的客户有"
                            + (s8.size()+clueMap.size()) + "组;");
                    zsMapResBOList.addAll(s8);
                    zsMapResBOList.addAll(clueMap);
                }else{
                    if (!CollectionUtils.isEmpty(s8) && s8.size() > 0) {
                        str.append(ClueAcquisitionStatusEnum.getNameByStatus("8") + "的客户有"
                                + s8.size() + "组;");
                        zsMapResBOList.addAll(s8);
                    }
    ...
    private List<String> initClueStatus(String clueStatus, String taskName, String flagType) {
        ...
        if (StringUtils.isNotBlank(flagType)) {
            if ("1".equals(flagType)) {//在保客户
                clueStatusList = Stream.of("1", "2", "4", "6", "7","8").collect(Collectors.toList());
            }
            if ("3".equals(flagType)||"4".equals(flagType)) {//地图导入客户和线索客户
                clueStatusList = Stream.of("3","5", "8", "9").collect(Collectors.toList());
            }
        }
